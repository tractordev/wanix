...
FS.prototype.Mount = function(path, fs)
{
    ...
    {
        console.debug("Mount failed: parent for path not found: " + path, LOG_9P);
        return -ENOENT;
    }
    if(path_infos.id !== -1)
    {
        console.debug("Mount failed: file already exists at path: " + path, LOG_9P);
        return -EEXIST;
    }
...
FS.prototype.OpenInodeAsync = async function(id, mode)
{
    var inode = this.inodes[id];
    if(this.is_forwarder(inode))
    {
        return await this.follow_fs(inode).OpenInodeAsync(inode.foreign_id, mode);
    }

    return this.OpenInode(id, mode);
};

FS.prototype.SearchAsync = async function(parentid, name)
{
    const parent_inode = this.inodes[parentid];
    ...
    {
        const foreign_parentid = parent_inode.foreign_id;
        const foreign_id = await this.follow_fs(parent_inode).SearchAsync(foreign_parentid, name);
        if(foreign_id === -1) return -1;
        return this.get_forwarder(parent_inode.mount_id, foreign_id);
    }
