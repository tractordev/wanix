  ...
  view: ({attrs: {store, inviteURL}}) => {
    return (
      <main style="width: 512px;" class="h-full mx-auto mt-64 border-4 border-black bg-white p-4 flex flex-col gap-3">
        <h1>
          <span class="text-3xl italic uppercase">Jazz Tdos</span>
          <button class="float-right px-2 bg-gray-300 rounded my-2" onclick={() => navigator.clipboard.writeText(inviteURL)}>Invite</button>
        </h1>

        <ul>
          {store.map((t,idx) =>
            <li key={t.id} class="flex flex-row">
              ...
              <div class="text-xs mr-2 py-1 font-normal">{t.creator}</div>
