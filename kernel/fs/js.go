...
func (s *Service) InitializeJS() {
	...
	for _, fn := range []string{"readdir", "stat", "readFile", "writeFile"} {
		js.Global().Get("sys").Call("handle", fmt.Sprintf("fs.%s", fn),
			js.Global().Get("duplex").Call("handlerFrom",
				js.Global().Get("api").Get("fs").Get(fn)))
	}
