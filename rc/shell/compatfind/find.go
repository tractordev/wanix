...
func (f *File) String() string {
	s := lsfmt.LongStringer{
		Human: true,
		Name:  lsfmt.NameStringer{},
	}
	rec := lsfmt.FromOSFileInfo(f.Name, f.FileInfo)
	rec.Name = f.Name
...
// WithRoot sets a root path for the file finder.
...
// WithFilenameMatch uses filepath.Match's shell file name matching.
...
// reorderArgs reorders arguments so flags are moved to the front.
